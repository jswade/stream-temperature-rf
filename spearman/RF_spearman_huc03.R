...
# Establish site indices located in HUC03
huc03_ind <- which(strat_huc$HUC02 == 3)


## Subset data by huc_ind
site_data <- site_data[huc03_ind,]
disch.month <- disch.month[huc03_ind,]
slope_metric <- slope_metric[huc03_ind,]
max_metric <- max_metric[huc03_ind,]
...
# # Write files to csv
write.csv(maxst_df,"/stream-temperature-rf/data_results/SpearmanRank/HUC_Subset/maxst_huc03_spearman.csv",row.names=FALSE)
write.csv(slope_df,"/stream-temperature-rf/data_results/SpearmanRank/HUC_Subset/slope_huc03_spearman.csv",row.names=FALSE)
