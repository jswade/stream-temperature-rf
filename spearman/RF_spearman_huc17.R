...
# Establish site indices located in HUC17
huc17_ind <- which(strat_huc$HUC02 == 17)


## Subset data by huc_ind
site_data <- site_data[huc17_ind,]
disch.month <- disch.month[huc17_ind,]
slope_metric <- slope_metric[huc17_ind,]
max_metric <- max_metric[huc17_ind,]
...
# # Write files to csv
write.csv(maxst_df,"/stream-temperature-rf/data_results/SpearmanRank/HUC_Subset/maxst_huc17_spearman.csv",row.names=FALSE)
write.csv(slope_df,"/stream-temperature-rf/data_results/SpearmanRank/HUC_Subset/slope_huc17_spearman.csv",row.names=FALSE)
